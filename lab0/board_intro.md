...
本次实验使用的 FPGA 芯片型号为安路 EG4S20BG256, 使用的开发板如下图所示，由一块核心板和一块底板连接而成.
...
LED 灯对应[上图](#平台介绍)中红圈 1 的部分. 本次实验使用的开发板提供8个直接由 FPGA 引脚驱动的 LED 灯 LED0 - LED7, 当 FPGA 输出高电平时, LED 灯点亮.
...
调试模块对应[上图](#平台介绍)中红圈 4 的部分和红圈 6 的部分. 在搭建基于 Cortex-M0 的 SoC 时, 调试系统是必不可少的. 为了方便使用, FPGA 开发板在设计之初便将调试电路封装在板上, 预留出来一个 JTAG 口和一个 DAP-LINK 口用于和 PC 机通信, 这样做不仅简化了对调试器内部原理的理解需求, 而且方便了连线. 调试器部分的端口需要约束的有一个时钟信号和一个双向的数据信号. 调试时需要注意.

> #### hint::为什么会有两个接口?
>
> 上面的（红圈6）的 DAP-LINK 的接口是用来调试 FPGA 中的 SoC 的（在Keil软件中）, 下面的（红圈4）的 JTAG 接口是用来下载比特流文件的（在TD软件中）. 
>
...
- 对于核心板的更多信息可以访问[芯片和板卡简介](https://www.yuque.com/yingmuketang/01/sz9qkx4rnuw6yctz)
